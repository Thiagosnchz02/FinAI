...
import '../styles/Login.scss';
import toast from 'react-hot-toast'; // Importar toast
import PhoneInput, { isValidPhoneNumber } from 'react-phone-number-input'; // Importar PhoneInput y isValidPhoneNumber
...
function Login() {
  ...
  const handleSignUpSubmit = useCallback(async (event) => {
    ...
    if (signUpData.phoneNumber && !isValidPhoneNumber(signUpData.phoneNumber)) {
            toast.error('El número de teléfono introducido no es válido.'); return;
        }

    setSignUpLoading(true);
    const toastId = toast.loading('Registrando...');
    try {
        const phoneToSubmit = signUpData.phoneNumber ? signUpData.phoneNumber : undefined;
        console.log('[Login.jsx] Intentando registrar con teléfono:', phoneToSubmit, '(Original en estado:', signUpData.phoneNumber, ')'); // Log para depurar
        ...
        const { data, error } = await supabase.auth.signUp({
            ...
            options: { 
                ...
                phone: phoneToSubmit // Usar la variable preparada
            }
        });
        if (error) throw error;
        ...
        setSignUpData({ name: '', email: '', password: '', phoneNumber: '' }); // Limpiar form
