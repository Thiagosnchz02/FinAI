    ...
    <div className="modal-overlay active" style={{ display: 'flex' }} onClick={(e) => { if (e.target === e.currentTarget && !isSaving) onClose(); }}>
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="input-group"> <label htmlFor="expenseCategoryM">Categoría</label> <select id="expenseCategoryM" name="categoryId" required value={formData.categoryId} onChange={handleChange} disabled={isSaving || categories.length === 0}><option value="" disabled>Selecciona...</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select> </div>
          <div className="input-group"> <label htmlFor="expenseAccountM">Cuenta de Cargo</label> <select id="expenseAccountM" name="accountId" value={formData.accountId} onChange={handleChange} disabled={isSaving}><option value="">(Opcional)</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select><small>Cuenta de la que saldrá (si aplica).</small> </div>
